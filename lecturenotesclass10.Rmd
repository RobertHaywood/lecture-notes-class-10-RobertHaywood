...
0.125 is the "true" value, 0.1216 is given by the simulation.
...
I think the first situation would occur 1/15 = 0.067.
I think the second would be 2/15.

```{r simulate_rolls}
dice <- c(1, 2, 3, 4, 5, 6)
two_rolls <- tibble(
  roll1 = sample(dice, size = 10000, replace = TRUE),
  roll2 = sample(dice, size = 10000, replace = TRUE))
two_rolls
```

```{r identify_events}
two_rolls <- two_rolls %>%
  mutate(square = roll1 * roll1 == roll2) %>%
  mutate(diff = abs(roll2 - roll1) == 4)
```
...
Based on these results I think I was right.
